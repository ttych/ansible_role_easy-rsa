...
if [ -r "${EASY_RSA_REQUEST_PKI_DIR}/reqs/${CN}.req" ]; then
    echo2 "request for ${CN} already exists at ${EASY_RSA_REQUEST_PKI_DIR}/reqs/${CN}.req"
    exit 1
fi
if [ -r "${EASY_RSA_REQUEST_PKI_DIR}/private/${CN}.key" ]; then
    echo2 "key for ${CN} already exists at ${EASY_RSA_REQUEST_PKI_DIR}/private/${CN}.key"
    exit 1
fi
